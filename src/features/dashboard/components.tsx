...
export function BirthdaysCard({
  ...
  data: DashboardSummary['birthdays']
}) {
  const empty = data.today.length === 0 && data.thisWeek.length === 0
  return (
    <Card>
      <CardHeader className="border-b">
...
export function AnniversariesCard({
  ...
  data: DashboardSummary['anniversaries']
}) {
  const empty = data.today.length === 0 && data.thisWeek.length === 0
  return (
    <Card>
      <CardHeader className="border-b">
        <CardTitle>Marriage anniversaries</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        {empty ? (
          <p className="text-sm text-muted-foreground">
          ...
          <>
            {data.today.map((a) => (
              <div key={a.couple} className="flex items-center gap-3">
                <HugeiconsIcon
            ...
            {data.thisWeek.map((a) => (
              <div key={a.couple} className="flex items-center justify-between">
                <span className="flex items-center gap-3">
                  <HugeiconsIcon
                    icon={AnniversaryIcon}
                    className="size-4 text-muted-foreground"
                  />
                  <span className="text-sm text-foreground">{a.couple}</span>
                </span>
